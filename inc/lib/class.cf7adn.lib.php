	...
	class CF7ADN_Lib {
				...
				if ( !empty( $response ) ) {
					...
					if ( $response->getMessages()->getResultCode() == 'Ok' ) {
						...
						if ( $tresponse != null && $tresponse->getMessages() != null ) {
							...
							add_filter( 'wpcf7_mail_tag_replaced', function( $replaced, $submitted, $html, $mail_tag ) use ( $tresponse, $invoice_no ) {

								if ( 'authorize' == $mail_tag->corresponding_form_tag()->basetype ) {
		...
		*/
		...
		function wpcf7_authorize_validation_filter( $result, $tag ) {
			...
			if ( empty( $creditCard ) || !is_numeric( $creditCard ) || strlen( $creditCard ) != 16 ) {
			...
			if ( empty( $cvv_number ) || !is_numeric( $cvv_number ) || strlen( $cvv_number ) !== 3 ) {
